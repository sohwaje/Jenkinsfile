...
stage('Test') {
    if (useTest) {
      ...
      try {
            sh '/opt/maven/apache-maven-3.6.2/bin/mvn test -Pproduction'
            /* SLACK Configuration */
...
stage('Deploy') {
    if (useDeploy) {
        ...
        try {
            sh 'sed -i "s/hiclass.azurecr.io\\/hi-class-api.*/hiclass.azurecr.io\\/hi-class-api:${BUILD_NUMBER}/g" hiclass-pro-api.yml'
            println "Replace End"
